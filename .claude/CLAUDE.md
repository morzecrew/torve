...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-28 (commit b0230a6). Confidence: 100%.
...
### Code health
Three co-equal signals: defect risk 7.7/10 avg, hotspot health 6.14/10 (stable), worst `src/torve/cli/merge.py` at 1.02/10 · maintainability 9.05/10 · performance risk 114 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `tests/test_shadow.py` — prior defect — impact −2.0
- `tests/test_broker.py` — prior defect — impact −2.0
- `src/torve/gates/source_layout.py` — untested hotspot — impact −2.0
- `src/torve/cli/status.py` — untested hotspot — impact −2.0
