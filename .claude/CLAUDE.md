...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-02 (commit 33d3032). Confidence: 100%.
...
- `src/torve/application/runner.py` — 9 bug fixes, last fix today (bug magnet); 47 commits/90d
- `src/torve/application/tracker.py` — 4 bug fixes, last fix 4 days ago (bug magnet); 23 commits/90d
- `tests/test_tracker.py` — 4 bug fixes, last fix 4 days ago (bug magnet); 18 commits/90d
- `src/torve/application/review.py` — 3 bug fixes, last fix today; 18 commits/90d

### Code health
Three co-equal signals: defect risk 8.02/10 avg, hotspot health 6.77/10 (stable), worst `src/torve/application/intake.py` at 2.0/10 · maintainability 9.15/10 · performance risk 157 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `src/torve/cli/fleet.py` — large method (_build_deps) — impact −1.5
