...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-28 (commit b0d3246). Confidence: 100%.
...
- `tests/test_shadow.py` — 3 bug fixes, last fix 6 days ago; 5 commits/90d
- `src/torve/application/runner.py` — 2 bug fixes, last fix today; 24 commits/90d

### Code health
Three co-equal signals: defect risk 7.72/10 avg, hotspot health 4.71/10 (stable), worst `src/torve/cli/merge.py` at 1.02/10 · maintainability 9.05/10 · performance risk 159 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `tests/test_shadow.py` — prior defect — impact −2.0
- `tests/test_broker.py` — prior defect — impact −2.0
- `src/torve/cli/run.py` — untested hotspot — impact −2.0
- `src/torve/cli/rfc.py` — untested hotspot — impact −2.0
