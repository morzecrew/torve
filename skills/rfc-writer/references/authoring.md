...
Read `references/rfc-template.md` before writing and start from `torve rfc
new`. A document is YAML: the header keys carry the facts (`status`,
`implementation`, `depends_on`, `owner`, `description`), `sections` carries
the prose in order, and the typed lists carry what the machine reads. The
...
5. **Design** — the core; subsections per workstream or component inside the body, with real signatures/schemas/code blocks where they pin the design. Where a choice was contested, record the rejected alternative and why it lost in the `alternatives` list — the trade-off, not just the rejection
...
- **Prose is prose.** Markdown inside a section body is welcome — headings, fences, links — and nothing parses it. Type what was already a list; never fragment an argument into fields.
