...
The draft is one document per adoption, the directory `S-NNNN/`
in the corpus path (`.torve/specs/` by default) — the shape this skill records.
Allocate the number with `torve spec new "Standing decisions"` when the
...
`fixtures/survey-report.json` and `fixtures/S-0001/` ride the
