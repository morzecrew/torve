...
files split by who writes each (RFC 0057, D-57.1):
...
the checks — belongs to the package (RFC 0007 §3a, D-7.12); `torve spec`
...
4. **Decision identifiers are permanent.** Divergence logs and code comments
   cite `D-3.1` forever; renumbering orphans every one of them. Append new
...
5. **Amendments, not edits.** An accepted document is amended — an entry in
   its `amendments.yaml` (globally numbered `A-n`, charter D-A.5) carrying the
...
| `invariants` | decisions | `id` (`I-NNNN.n`), `statement`, `paths`, `check` | a rule with the command that proves it |
...
| `questions` | document | `id` (`Q-NNNN.n`), `text`, `status` open/settled, `settled_by` | what the design leaves for someone to settle |
...
amendment's words is a `check` problem (D-57.2) — the list exists once.
...
edited by hand is editorial drift — a warning — and `torve spec fix D-x.y
...
D-44.12` answers marked archived, a citation into it checks clean, and the
