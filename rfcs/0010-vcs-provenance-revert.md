...
| --- | --- | --- | --- |
| D-10.1 | `LOCKED` | The agent produces code; the runner produces every forge artefact | The mechanism that makes D-4b achievable |
| D-10.2 | `LOCKED` | Commit author is the agent identity, never a human | Attributing machine work to a person corrupts blame and review data |
| D-10.3 | `LOCKED` | Signing happens outside the sandbox, at the runner boundary | The agent cannot hold the key |
| D-10.4 | `LOCKED` | Provenance trailers on every commit, including `config_hash` | The only durable link from a bad commit back to its regime |
| D-10.5 | `LOCKED` | Force-push forbidden once review has started | Protects review freshness and line comments |
| D-10.6 | `LOCKED` | Pull-request bodies are composed from data, never from agent prose | A self-report is not evidence |
| D-10.7 | `LOCKED` | Revert is a role, preferring `git revert`, passing the same gates and the same lane | An unreviewed emergency path is how the guarantees get bypassed |
| D-10.8 | `ASSUMED` | One commit per attempt | Depart if attempts routinely produce unrelated changes — which is itself a task-size finding |
