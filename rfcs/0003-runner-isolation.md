...
- **Implementation state:** complete (judged 2026-08-23, when the D-3.6 Postgres regime went live in dogfood). Phases 1–2 shipped 2026-08-21 (T-0003 runner core and both runtime adapters; T-0004 durable store facade with leases/fencing/cancellation, recovery-driven reap, DST simulation with broken twins). the pull-request leg executed 2026-08-23 (T-0045 with RFC 0010); transactional notifications executed 2026-08-23 (T-0051 with RFC 0006 — D-3.18 closed by D-6.11). The lab's real runs now execute over a migrated Postgres store (provisioned per its ops/README; `torve doctor` checks DSN, reachability and schema currency since T-0062, and the tick's reap leg carries the store factory since T-0063), with the full dispatch → approvals-short refusal → approve → land cycle exercised over durable run records. Live OpenSandbox server integration achieved 2026-08-31 against a self-hosted opensandbox-server (0.1.15, Docker runtime, API-key auth enforced): T-0152 plus operator triage reshaped the adapter and its stub to the real SDK surface, and the full Runtime contract was live-verified — create with tar-seed, exec under the native working directory, labelled enumeration, sync_out round-trip with `.git` excluded both directions, destroy, and the platform-enforced timeout observed collecting a sandbox. The vault and egress capabilities (D-21.2's broker adapter, D-17.10's docker question) remain unexplored — the server exists; those decisions now wait on their own evidence, not on infrastructure
...
Not included, deliberately: other tasks' state, other tasks' escalations, the divergence logs of adjacent work. An executor that can read them is an executor that can argue its way out of its own scope (D-31).
...
| D-3.18 | `ASSUMED` | Transactional notifications and the delivered-notification simulation invariant land with RFC 0006, where the Notifier policy lives. Added by execution 2026-08-21. *Executed 2026-08-23 (T-0051, RFC 0006 D-6.11):* the notify effect rides the outbox keyed (task, attempt, reason); the exactly-once-under-replay invariant is pinned by test | `src/torve/application/tracker.py` | Until then escalations are visible through `torve status` only |
