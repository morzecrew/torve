...
| D-34.2 | `ASSUMED` | A Phasing entry may carry `character:` with value `structural` or `routine`, validated at parse, copied verbatim by the planner onto the contract; absent means undeclared and is never inferred | `src/torve/config/rfc_parse.py` `src/torve/application/planner.py` `src/torve/domain/task.py` | — |
...
| D-34.4 | `ASSUMED` | A gate declaration may carry `axis:` with value `functional`, `boundary`, `compliance` or `form`; an unlabeled gate reads as functional | `src/torve/config/manifest.py` `.torve/gates.yaml` | The fail-safe default routes an unlabeled gate's retry to the heaviest rung — a missing label costs money, never correctness |
