...
| D-8.11 | `ASSUMED` | A landed task's issue is closed by a landings pass: the run state is swept after the landing, so sync consults the landing trailer for tasks with no live state and stages one close-out effect per task, ever; the adapter closes an existing issue and never creates one just to close it. Added by execution 2026-08-24 — see .torve/tasks/T-0065 | `src/torve/application/tracker.py` `src/torve/adapters/tracker/github.py` | Under unattended operation, landed issues lingered open with stale state labels — a board nobody grooms must groom itself |
