...
def test_without_skill_strips_every_role_set():
    ...
    assert all("flag-dont-flip" not in names
               for names in stripped.skills.sets.values())
...
def test_skill_eval_runs_both_arms_and_ledgers(repo):
    ...
    config = RunnerConfig(runtime=RuntimeConfig(sandbox_timeout=300, agent_timeout=90),
                          poison_ceiling=2)
    deps = RunDeps(
        workspace=GitWorkspace(repo.root), runtime=DockerRuntime(),
        agent=FakeAgent([{"writes": {"src/feature.py": "FEATURE = 'a'\n"}, "exit": 0},
    ...
    record = run_skill_eval(repo.root, "flag-dont-flip", [task],
                            config, deps, source)
    ...
    skills_by_shadow = [line["agent"]["skills"] for line in lines
