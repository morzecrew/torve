...
| D-7.12 | `LOCKED` | The RFC format is owned by the package; validation does not depend on an installed skill. From RFC 0053 the owner is the model's loader in `src/torve/config/spec.py`, with the parser in `src/torve/config/rfc_parse.py` beside it until phase 5 deletes it (D-53.13) | `src/torve/config/rfc_parse.py` | Two parsers and no definition is how the planner and the validator drift apart |
...
| D-7.21 | `ASSUMED` | The Phasing format is a fenced YAML unit list under `## Phasing`, owned by `rfc_parse` like the rest of the format; prose-only sections are legal but unmintable. Added by execution 2026-08-22 — see .torve/tasks/T-0025 | `src/torve/config/rfc_parse.py` `skills/rfc-writer/references/rfc-template.md` | — |
| D-7.22 | `ASSUMED` | Minting mechanics: the full decision table is inherited with grade and paths at write time; task ids are the maximum plus one and never reused; an already-minted phase refuses; same-phase scope overlap refuses conservatively. Added by execution 2026-08-22 — see .torve/tasks/T-0025 | `src/torve/application/planner.py` | — |
| D-7.23 | `ASSUMED` | Phase scopes may overlap across phases — dependency order serialises them; only scopes within one phase must be disjoint. Owner-settled from the T-0028 review. Added by execution 2026-08-22 — see .torve/tasks/T-0028 | `src/torve/application/planner.py` | — |
| D-7.24 | `ASSUMED` | Proposal freshness is derived from corpus citation of the originating task id; promoted rows keep citing their task precisely because this projection reads it. Added by execution 2026-08-22 — see .torve/tasks/T-0026 | `src/torve/application/projections.py` `rfcs/**` | — |
...
### A-151 — 2026-09-09 — the format's owner is the loader

**By RFC 0053 phase 3.** The rule stands — the format is the package's
and needs no skill — and its owner moves: the model's loader in
`src/torve/config/spec.py` is what every reader consumes (D-53.1), the
parser stays beside it with archive awareness (A-140) until phase 5
deletes it, and the import-linter contract that keeps the format out of
gates and adapters follows the owner then.
```yaml changes
- subject: "D-7.12"
  field: "text"
  before: "The RFC format is owned by the package; validation does not depend on an installed skill"
  after: "The RFC format is owned by the package; validation does not depend on an installed skill. From RFC 0053 the owner is the model's loader in `src/torve/config/spec.py`, with the parser in `src/torve/config/rfc_parse.py` beside it until phase 5 deletes it (D-53.13)"
- subject: "D-7.12"
  field: "fingerprint"
  before: null
  after: "5f4218d6a9f7f40e/71ed077d6d777502"
```
