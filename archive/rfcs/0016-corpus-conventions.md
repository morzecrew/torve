...
supersedes: []
superseded_by: "0055"
amended_by: ["A-7", "A-9", "A-10", "A-14", "A-15", "A-20", "A-53", "A-148", "A-149", "A-150"]
owner: Lev Litvinov
description: >-
...
schema_version: 1
fingerprints:
  D-A.17: 67ad88677aa44b9a/71ed077d6d777502
  D-A.18: b3e0b83d1881d805/bcecc04abf43a1cb
  D-A.19: b3c7cde278428747/a0d73f2ad31523c3
---
...
| D-A.17 | `LOCKED` | The next number is derived as the maximum plus one over the corpus path and the archive together, never stored in a counter file (RFC 0053 D-53.10). Added by amendment A-15 2026-08-22; amended by A-150 2026-09-09 | `src/torve/config/rfc_parse.py` | A counter is state; two branches diverge it and the resolution gives two documents one number |
| D-A.18 | `LOCKED` | Only `NNNN-slug.md` and `INDEX.md` in the corpus directory, no subdirectories; the check routes offenders to `pages/` or `ops/`. A retired document's one legal destination is `archive/rfcs/` beside the corpus path, keeping its filename and identifiers, written only by `torve rfc archive` (RFC 0053 D-53.8). Added by amendment A-15 2026-08-22; amended by A-148 2026-09-09 | `rfcs/**` `src/torve/config/rfc_parse.py` | Without routing the file lands in the repository root and the mess has moved rather than gone |
| D-A.19 | `LOCKED` | Documents are never deleted; identifiers are never reused; gaps are acceptable. A document moved into the archive is not deleted: every identifier it defines still resolves through `show`, the check and the record (RFC 0053 D-53.9). Added by amendment A-15 2026-08-22; amended by A-149 2026-09-09 | `rfcs/**` | Amendments, logs and commit trailers cite identifiers, and reuse redirects all of them silently |
...
| D-16.2 | `ASSUMED` | The decision-table column order is fixed and the validator enforces the exact header; a document's example tables are illustrative, never a reordering instruction. Added by execution 2026-08-22 — see .torve/tasks/T-0009 | `rfcs/**` `src/torve/config/rfc_parse.py` | — |
| D-16.3 | `ASSUMED` | LOCKED paths globs are repository-checked only for documents implemented beyond `none`; an unbuilt document names intended modules and refines them when they exist (D-32). Added by execution 2026-08-22 — see .torve/tasks/T-0009, refined at 0004's acceptance | `src/torve/config/rfc_parse.py` `rfcs/**` | — |
| D-16.4 | `ASSUMED` | Paths cells follow the code and relocate mechanically, carrying their current cells rather than a source patch's snapshot; an in-row marker only where the decision text itself names the superseded location. Added by execution 2026-08-22 — see .torve/tasks/T-0012, T-0016 and T-0017 | `rfcs/**` | — |
...
### A-148 — 2026-09-09 — the archive is where a retired document goes

**By RFC 0053 phase 3.** D-A.18 admitted nothing in the corpus directory
but documents and the index, and had no answer for a document that no
longer stands except leaving it in place. RFC 0053 D-53.8 gives a retired
document one destination — `archive/rfcs/` beside the corpus path, the
same filename, every identifier kept, `status: superseded` — reached only
through `torve rfc archive`, whose transaction refuses the move unless
the corpus without the document checks clean. The corpus directory rule
is unchanged; the archive is outside it.
```yaml changes
- subject: "D-A.18"
  field: "text"
  before: "Only `NNNN-slug.md` and `INDEX.md` in the corpus directory, no subdirectories; the check routes offenders to `pages/` or `ops/`. Added by amendment A-15 2026-08-22"
  after: "Only `NNNN-slug.md` and `INDEX.md` in the corpus directory, no subdirectories; the check routes offenders to `pages/` or `ops/`. A retired document's one legal destination is `archive/rfcs/` beside the corpus path, keeping its filename and identifiers, written only by `torve rfc archive` (RFC 0053 D-53.8). Added by amendment A-15 2026-08-22; amended by A-148 2026-09-09"
- subject: "D-A.18"
  field: "fingerprint"
  before: null
  after: "b3e0b83d1881d805/bcecc04abf43a1cb"
```

### A-149 — 2026-09-09 — moved is not deleted

**By RFC 0053 phase 3.** "Never deleted" was written when the only way a
document could leave was `git rm`. A document moved into the archive is
not deleted: the check resolves what it defines (A-140), `show` answers
its identifiers marked archived, and the record holds every row as
retired with the archive named as the reason (D-53.9). The row's second
sentence says so; the rule it protects — that a cited identifier always
resolves — is the one that made the archive necessary rather than a
`git rm`.
```yaml changes
- subject: "D-A.19"
  field: "text"
  before: "Documents are never deleted; identifiers are never reused; gaps are acceptable. Added by amendment A-15 2026-08-22"
  after: "Documents are never deleted; identifiers are never reused; gaps are acceptable. A document moved into the archive is not deleted: every identifier it defines still resolves through `show`, the check and the record (RFC 0053 D-53.9). Added by amendment A-15 2026-08-22; amended by A-149 2026-09-09"
- subject: "D-A.19"
  field: "fingerprint"
  before: null
  after: "b3c7cde278428747/a0d73f2ad31523c3"
```

### A-150 — 2026-09-09 — the next number counts the archive

**By RFC 0053 phase 3.** A number retired into the archive is still a
number that was cited, so the maximum the next one derives from runs
over the corpus path and the archive together (D-53.10). `torve rfc new`
reads the loader's derivation; a gap left by an archived document stays a
gap.
```yaml changes
- subject: "D-A.17"
  field: "text"
  before: "The next number is derived as the maximum plus one, never stored in a counter file. Added by amendment A-15 2026-08-22"
  after: "The next number is derived as the maximum plus one over the corpus path and the archive together, never stored in a counter file (RFC 0053 D-53.10). Added by amendment A-15 2026-08-22; amended by A-150 2026-09-09"
- subject: "D-A.17"
  field: "fingerprint"
  before: null
  after: "67ad88677aa44b9a/71ed077d6d777502"
```
