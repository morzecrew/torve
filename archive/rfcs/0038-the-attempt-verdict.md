...
owner: Lev Litvinov
description: >-
  Every attempt ends in exactly one durable telemetry row stamped with its attempt number and an engine-derived verdict, and every escalation lands an engine event — how a run ended stops dying with the state file.
schema_version: 1
---
